...
static void blink_led(void)
{
    ...
    if (s_led_state) {
        ...
        led_strip_set_pixel(led_strip, 1, 16, 0, 16);
        led_strip_set_pixel(led_strip, 2, 16, 16, 0);
...
void led_task(void *pvParameters) {
    while (1) {
         ...
         if( 0 == (wifi_connected || etg_connected)){
            ...
            s_led_state = !s_led_state;
            vTaskDelay(CONFIG_BLINK_PERIOD / portTICK_PERIOD_MS);
        }else if((BT_CONN_WRITE_END == m_bt_state) || (BT_CONN == m_bt_state)){
...
void led_init(void)
{
    ...
    configure_led();
    xTaskCreate(led_task, "led_task", 1024*5, NULL, configMAX_PRIORITIES-10, NULL);
}
